...
export default function InviteCodeModal({ project, onSuccess, onClose }: InviteCodeModalProps) {
  ...
  const handleSubmit = () => {
    ...
    if (!trimmed) return;
    const ok = joinProject(project.id, trimmed);
    if (ok) {
      onSuccess();
    } else {
      setError(true);
      ...
      >
        ...
        <div className="flex flex-col items-center pt-8 pb-5 px-6 border-b border-slate-800 bg-slate-950/50">
          <ProjectAvatar
            ...
            size="lg"
          />
          <h2 className="mt-3 text-base font-semibold text-white">{project.name}</h2>
